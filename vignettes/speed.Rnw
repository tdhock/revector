...
works_with_R("3.0.2", revector="2014.2.10", microbenchmark="1.3.0",
             ggplot2="0.9.3.1", dplyr="0.1")
data(genotypes)
...
times <- data.frame()
set.seed(1)
for(size in c(5, 10, 50, 100, 500, 1000)){
  print(size)
  picked <- sample(nrow(complicated), size)
  geno.data <- complicated[picked,]
  bench <- do.call(microbenchmark, margs)
